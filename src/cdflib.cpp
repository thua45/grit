...
void mixed_ttest(double &one, vector<double> &obs, vector<double> &bkg, double &pvalue, double &tvalue, double &dfvalue, double &ccv, double &sd) {
  if (bkg.size() < 2 || obs.size() < 2) {
    pvalue = 1.0;
    tvalue = -1;
    dfvalue = -1;
    ccv = 0.0;
  ...
  cov(bkg, cov_bkg, mean_bkg);
  cov_one(one, obs, cov_obs, mean_obs);
  ...
  //t = (mean_obs - mean_bkg) / sqrt(cov_obs / obs_n + cov_bkg / bkg_n);
  //detal_S = sqrt(cov_obs / (double) obs_n + cov_bkg / (double) bkg_n);
  //t = (mean_obs - mean_bkg) / sqrt(((obs_n - 1) * cov_obs + (bkg_n - 1) * cov_bkg) / (obs_n + bkg_n - 2) * (1.0 / obs_n + 1.0 / bkg_n));
  //detal_S = sqrt(((obs_n - 1) * cov_obs + (bkg_n - 1) * cov_bkg) / (obs_n + bkg_n - 2) * (1.0 / obs_n + 1.0 / bkg_n));
  //detal_S = sqrt(cov_obs / (double) obs_n + cov_bkg / (double) bkg_n);
  // if (detal_S > 0) {
  //   t = (one - mean_bkg) / detal_S;
  // }
  // else {
  //   t = -1.79e308;
  // }

  t = (mean_bkg - one) / sqrt(((obs_n - 1) * cov_obs + (bkg_n - 1) * cov_bkg) / (obs_n + bkg_n - 2) * (1.0 / obs_n + 1.0 / bkg_n));

  // detal_f = pow(cov_obs / (double) obs_n, 2.0) / ((double) obs_n - 1.0) + pow(cov_bkg / (double) bkg_n, 2.0) / ((double) bkg_n - 1.0);

  // if (detal_f > 0) {
  //   df = pow((cov_obs / (double) obs_n + cov_bkg / (double) bkg_n), 2.0) / detal_f;
  // }
  // else {
  //   df = -1.79e308;
  // }

  df = pow((cov_obs / (double) obs_n + cov_bkg / (double) bkg_n), 2.0) / (pow(cov_obs / (double) obs_n, 2.0) / ((double) obs_n - 1.0) + pow(cov_bkg / (double) bkg_n, 2.0) / ((double) bkg_n - 1.0));
  //cout << t << endl;

  //int ifault;
  //double pvalue = 1.0 - tnc(t, df, nctp, &ifault);
  ...
  if (cov_obs != 0.0) {
    ccv = abs(mean_obs) / sqrt(cov_obs);
  } else {
    ccv = 0.0; // not conserved
  }
  if (mean_bkg != 0.0) {
    sd = (one - mean_bkg) / abs(mean_bkg);
  } else {
    sd = 0.0; // no difference
  }

}

void cov(vector<double> &data, double &covc, double &meanr) {
  if (data.size() == 0) {
    covc = 0.0;
    meanr = 0.0;
    return;
  ...
  double cov_cum = 0.0;
  for (unsigned int i = 0; i < data.size(); ++i) {
    cov_cum += pow((data[i] - mean), 2.0);
  }
  //return cov_cum;
  covc = cov_cum / (double) data.size();
  meanr = mean;

}

void cov_one(double &one, vector<double> &data, double &covc, double &meanr) {
  if (data.size() == 0) {
    covc = 0.0;
    meanr = 0.0;
    return;
  ...
  meanr = total / (double) data.size();

  double cov_cum = 0.0;
  for (unsigned int i = 0; i < data.size(); ++i) {
    cov_cum += pow((data[i] - one), 2.0);
  }
  covc = cov_cum / (double) data.size();
